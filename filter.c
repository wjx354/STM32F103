...
* 备  注 : 无
********************************************************************************/
void SortAver_FilterEluer_hat(Eluer_hat *eluer_hat,Eluer_hat *eluer_hat_filt,uint8_t n)
{
	static float bufx[N],bufy[N],bufz[N];
	static uint8_t cnt =0,flag = 1;
	float temp1=0,temp2=0,temp3=0;
	uint8_t i;
	bufx[cnt] = eluer_hat->a_p_hat;
	bufy[cnt] = eluer_hat->a_r_hat;
	bufz[cnt] = eluer_hat->a_y_hat;
	cnt++;      //这个的位置必须在赋值语句后，否则bufx[0]不会被赋值
	if(cnt<n && flag) 
		return;   //数组填不满不计算
	else
	...
	for(i=1;i<n-1;i++)
	 ...
	 if(cnt>=n) cnt = 0;
	 eluer_hat_filt->a_p_hat = temp1/(n-2);
	 eluer_hat_filt->a_r_hat = temp2/(n-2);
	 eluer_hat_filt->a_y_hat = temp3/(n-2);
}
/*******************************************************************************
